...
export default function OrganizerEmailVerification(){
    ...
    useEffect(() =>{
        ...
        if(storedEmail && storedOrganizerid) {
            ...
            alert("email or organizer id is not found.");
    ...
    const handleVerifyOtp = async() =>{
        ...
        try{
            ...
            if (!email || !organizerid) {
                throw new Error("Email is or organizer id is null");
            }
            const response = await axios.post('http://localhost:8080/verify-otp',{email,otp});
            if(response.data.message === 'OTP verified'){
                window.location.href=`/organizer-change-pw`;
            }
            else{
